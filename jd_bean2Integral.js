...
class UserClass {
        ...
        try {
            ...
            if(paramOut.result && typeof paramOut.result === 'object') {
                ...
                if(result.success) {
                    ...
                    console.log(`获取账户信息失败：${result?.error_response?.zh_desc}`)
                }
            }
        } catch(e) {
            console.log(e)
    ...
    async queryexpirejingdou(paramIn={}) {
        let paramOut = {}
        try {
            let urlObject = {
            ...
            let taskApiParam = {
                fn : 'queryexpirejingdou',
                method : 'get',
                urlObject : urlObject,
            }
            paramOut = await this.taskApi(taskApiParam)
            if(paramOut.result) {
                function QueryExpireJingdou(obj) {
                    return obj;
                }
                let exprireConfig = eval(paramOut.result);
                let sevenDayAmount = 0
                let threeDayAmount = 0
                let nowtime = Date.now()
                for(let item of exprireConfig.expirejingdou) {
                    let amount = Number(item.expireamount)
                    if(amount > 0) {
                        let expireTime = item.time*1000
                        //let timestr = $.time({format:'MM月dd日过期京豆',time:expireTime})
                        //console.log(`${timestr}: ${amount}`)
                        sevenDayAmount += amount
                        if(expireTime - nowtime < 3*24*60*60*1000) {
                            threeDayAmount += amount
                        }
                    }
                }
                //console.log(`三天内过期京豆${threeDayAmount}，七天内过期京豆${sevenDayAmount}`)
                let beanAmount = 0
                if(threeDayAmount > 0) {
                    beanAmount = $.getMax(sevenDayAmount,100)
                    beanAmount = $.getMin(beanAmount,5000)
                    //console.log(`账号${this.index}[${this.name}]三天过期${threeDayAmount}，七天过期${sevenDayAmount}，兑换${beanAmount}京豆到积分`)
                } else {
                    console.log(`账号${this.index}[${this.name}]三天内没有过期京豆，不兑换`)
                }
                if(beanAmount > 0) {
                    await this.transfer({mode:'bean2Integral',amount:beanAmount})
                }
            }
        } catch(e) {
            console.log(e)
